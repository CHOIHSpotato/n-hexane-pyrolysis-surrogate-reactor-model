...
# Emit the predict possibility of some heavy species (ex. C4+) for better convergence
i_obs = torch.arange(0, ns - 2, device=device)
#i_obs = torch.arange(0, ns, device=device)
#_obs = i_obs[i_obs != 5]  # Remove the element 5
...
# LLNL
file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_on/LLNL_Eon_{i + 1}.txt" for i in range(n_exp)]

# NUIG
#file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_on/NUIG_Eon_{i + 1}.txt" for i in range(n_exp)]

# JetSurf
#file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_on/JetSurf_Eon_{i + 1}.txt" for i in range(n_exp)]
...
# Generate temperature profile from trained temp. predict ANN using initial conditions
input_node = 2 # initial temperature, and initial pressure 
output_node = ntotal-1 # Number of steps - 1 (except initial temperature)
neurons = 512

T_ini_list = Tlist[:,0]
P_ini_list = Plist[:,0]

load_dir = 'C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/TEMP_PRED_MODEL_PARAMETER_CONTAINER'

input_params = np.zeros((n_exp, input_node))
input_params = torch.stack((T_ini_list, P_ini_list), dim=1)

input_scale = np.asarray([[870, 1.0e+5],
                        [1150, 3.0e+5]])  # Initial temperature and pressure

with open(f'{load_dir}/min_max_values_mlp_LLNL_2D.pkl', 'rb') as f:
    output_scale = pickle.load(f)

# Extract min and max values
min_val = output_scale['min']
max_val = output_scale['max']

input_params[:,0] = (input_params[:,0]  - input_scale[0,0])/(input_scale[1,0] - input_scale[0,0]) # initial temperature
input_params[:,1] = (input_params[:,1]  - input_scale[0,1])/(input_scale[1,1] - input_scale[0,1]) # initial pressure

print(f"input_params: {input_params}, {input_params.size()}, {type(input_params)}")

# Calculate the tempreature profile from the trained NN model
...
model.load_state_dict(torch.load(f"{load_dir}/mlp_weights_LLNL_2D.pth", 
                                 map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))

model.eval()  # set to evaluation mode

with torch.no_grad():
    predicted_profile_scaled = model(input_params) 
pred_profile_scaled = predicted_profile_scaled.squeeze(0).cpu().numpy()

# Inverse transform
pred_profile = predicted_profile_scaled * (max_val - min_val) + min_val
print(f"pred_profile: {pred_profile.size()}, {type(pred_profile)}")
T_ini_list_reshape = T_ini_list.unsqueeze(1)
Tlist_calculated = torch.cat((T_ini_list_reshape,pred_profile), dim=1)
print(f"Tlist: {Tlist.size()}, {type(Tlist)}")
print(f"Tlist_calculated: {Tlist_calculated.size()}, {type(Tlist_calculated)}")

temp_error = torch.mean(torch.abs(Tlist - Tlist_calculated), dim=1, keepdim=True)

#######################################################################################
## Activate it when the surrogate model predict the results at Eon
Tlist = Tlist_calculated
mean_temp_error = torch.mean(temp_error)
print(f"mean_temp_error: {mean_temp_error}")
#######################################################################################
...
with torch.no_grad():
    ...
    p[nr*2] = p[nr*2] * (1/((Ea_fit+A_fit+ns+nr)/(Ea_fit-ns-nr)))
...
def ParameterConverter(p):
        ...
        slope_Ea = Ea_fit * ((Ea_fit+A_fit+ns+nr)/(Ea_fit-ns-nr))
...
scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, threshold=1e-4, threshold_mode='rel')
crnn = CRNN(tsteps)

trainer = Trainer(tsteps, crnn, ylabel, yscale, u0_list, Tlist, Plist, i_obs)
epochs = num_epochs
save_path = "C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/SURROGATE_MODEL_PARAMETER_CONTAINER/training_history_LLNL_Eon_v4.npz"
