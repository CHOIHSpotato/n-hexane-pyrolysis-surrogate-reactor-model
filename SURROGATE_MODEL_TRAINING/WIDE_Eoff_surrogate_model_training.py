...
grad_clip_value_set = 1.0 * 1.0e+1
learning_rate = 5.0 * 1.0e-4

## The value of b_fit is defined the minimum in the fitting results within 100 kPa - 300 Kpa,
## and the value of Ea_ift is defined the maximum in the fitting results within 100 kPa - 300 Kpa.
...
# LLNL
b_fit = torch.tensor(2.112, dtype=torch.float32, device=device, requires_grad=False) 
Ea_fit = torch.tensor(63.304, dtype=torch.float32, device=device, requires_grad=False)

# NUIG
#b_fit = torch.tensor(1.796, dtype=torch.float32, device=device, requires_grad=False)
#Ea_fit = torch.tensor(56.955, dtype=torch.float32, device=device, requires_grad=False)

# JetSurf
#b_fit = torch.tensor(1.826, dtype=torch.float32, device=device, requires_grad=False)
#Ea_fit = torch.tensor(55.792, dtype=torch.float32, device=device, requires_grad=False)

lb = torch.tensor(1.0e-6, dtype=torch.float32, device=device, requires_grad=False)
ub = torch.tensor(6.0e+1, dtype=torch.float32, device=device, requires_grad=False)
intermediate_min = torch.tensor(-1.0e+1, dtype=torch.float32, device=device, requires_grad=False)
intermediate_max = torch.tensor(1.0e+1, dtype=torch.float32, device=device, requires_grad=False)
...
#####################################################################################
# Define bounds
ll_wout, ul_wout = torch.tensor(-5.0, dtype=torch.float32, device=device), torch.tensor(5.0, dtype=torch.float32, device=device)
ll_win, ul_win = torch.tensor(0.0, dtype=torch.float32, device=device), torch.tensor(5.0, dtype=torch.float32, device=device)
ll_Ea, ul_Ea = torch.tensor(5.0, dtype=torch.float32, device=device), torch.tensor(200.0, dtype=torch.float32, device=device)
ll_b, ul_b = torch.tensor(-3.0, dtype=torch.float32, device=device), torch.tensor(3.0, dtype=torch.float32, device=device)
ll_A, ul_A = torch.tensor(1.0, dtype=torch.float32, device=device), torch.tensor(21.0, dtype=torch.float32, device=device)
...
# LLNL
file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_off_wide/LLNL_Eoff_{i + 1}.txt" for i in range(n_exp)]

# NUIG
#file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_off_wide/NUIG_Eoff_{i + 1}.txt" for i in range(n_exp)]

# JetSurf
#file_paths = [f"C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/CRNN_TEMP_PRED_MODEL_TRAINING_DATASET/fix_input_off_wide/JetSurf_Eoff_{i + 1}.txt" for i in range(n_exp)]
...
#tsteps = torch.arange(0, time_step * ntotal, time_step, device=device)
...
# Sample absolute values uniformly from [0.2, 1.0]
abs_values = torch.rand(size_p, device=device) * (1.0 - 0.2) + 0.2
...
decomp_fit = torch.tensor(0.4, device=device, requires_grad=False)
decomp_mul_fit = torch.tensor(0.5, device=device, requires_grad=False)
slope_reg = torch.tensor(0.5, device=device, requires_grad=False)
...
with torch.no_grad():
    ...
    p[0] = p[0] * (1/(slope_reg*A_fit/(A_fit+nr)))
    p[nr] = p[nr] * (1/(slope_reg*(A_fit+b_fit+nr)/(A_fit+b_fit+nr+ns)))
    p[nr*2] = p[nr*2] * (1/(slope_reg*(Ea_fit+A_fit+nr)/(Ea_fit-nr)))
    slope_A = A_fit * (A_fit/(A_fit+nr)) * slope_reg
    slope_b = b_fit * ((A_fit+b_fit+nr)/(A_fit+b_fit+nr+ns)) * slope_reg
    slope_Ea = Ea_fit * ((Ea_fit+A_fit+nr)/(Ea_fit-nr)) * slope_reg
    ...
    def wrapper(self, itpT, itpP, w_in, w_b, w_out, yscale):
        class CRNNFunc(nn.Module):
            ...
            def forward(self, t, u):
                ...
                if not torch.isfinite(w_in_x).all():
                    print(" w_in_x contains NaN or Inf")
                    print(w_in_x)
                    raise ValueError("Non-finite value in w_in_x")

                if not torch.isfinite(self.w_b).all():
                    print(" self.w_b contains NaN or Inf")
                    print(self.w_b)
                    raise ValueError("Non-finite value in w_b")
...
class Trainer:
    ...
    def predict_n_ode(self, p, i_exp):
        ...
        sol = odeint(crnn_func, u0, timelist_exp, method='dopri5', atol=1e-6, rtol=1e-4) # Adaptive time solver
    ...
    def test(self, p, test_idx):
        ...
        with torch.no_grad():
            ...
            for i_exp in test_idx:
                ...
                #fig.savefig(f"C:/CRNN/CRNN_train_server/result/local_output/LLNL_testset_wide/i_exp_{i_exp}.png")
                plt.close(fig)
...
optimizer = optim.AdamW([p], lr=learning_rate, weight_decay=1e-4)
scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=5, threshold=1e-4, threshold_mode='rel')
crnn = CRNN(tsteps)

trainer = Trainer(tsteps, crnn, ylabel, yscale, u0_list, Tlist, Plist, i_obs)
epochs = num_epochs
save_path = "C:/Users/KOREA/Desktop/Naphtha_ML_SOURCE_CODE/SURROGATE_MODEL_PARAMETER_CONTAINER/training_history_LLNL_wide_Eoff.npz"
